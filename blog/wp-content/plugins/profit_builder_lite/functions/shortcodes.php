...
function pbuilder_button($atts, $content = null) {
    ...
    switch ($btype) {
        ...
        case 'predone' :
            ...
            $html = '<div style="text-align:center;"><a href="' . $url . '"><img src="' . IMSCPB_URL . '/images/buttons/' . $imagename . '"></a></div>';
        ...
        case 'css3' :
            ...
            $html = '<div style="text-align:center;"><a class="pbcss3button' . $css3btnstyle . '" ' . $style . ' href="' . $url . '" ' . ($type == 'new-tab' ? ' target="_blank" ' : "") . '><span class="text" style="font-size:' . $font_size . ';">' . $text . '</span></a></div>';

            break;

        case 'image' :
            $html = '<div style="text-align:center;"><a href="' . $url . '"><img src="' . $image . '"></a></div>';
    }
...
//	if ($disablename=='false' && $formstyle != 'Horizontal')
    ...
    if ($formstyle == 'Horizontal') {
        ...
        $html.='<div class="field"><input type="text" validation="required email" class="validate[funcCall[checkoptinrequired],custom[email]]" error-message="' . $emailerror . '" name="email" value="' . $emailvalue . '" padding-left="10" padding-right="' . ($fieldbg == "true" ? "33" : "0") . '" style="padding-left:0px; font-size:' . ($fieldfontsize) . '; ' . ($fieldbg == "true" ? " background:url('" . $emailimage . "') no-repeat 98% center " . ($fieldbgtransparent == "true" ? "transparent" : $fieldbgcolor) . "; padding-right: 0px;   width:100%;" : 'background-color:' . ($fieldbgtransparent == "true" ? "transparent" : $fieldbgcolor)) . ';color:' . $fieldtextcolor . ' !important;" onfocus=" if (this.value == \'' . $emailvalue . '\') { this.value = \'\'; }" onblur="if (this.value == \'\') { this.value=\'' . $emailvalue . '\';} " default-value="' . $emailvalue . '" /></div>';
    }
    /*
     * Code added by Asim Ashraf - DevBatch
     * DateTime: 28 Jan 2015
...
//	$FieldwidthHori = $formstyle == 'Horizontal' ? "width: 49% !important; float: left !important; margin-right10px;" : "width: 100%";
    ...
    if (!empty($extension[0]['name']) && !empty($imscpbiw_access_response->access_token)) {
...
function pbuilder_fbcomments($atts, $content = null) {
    ...
    $permalink = $fb_source_url == '' ? get_permalink() : $fb_source_url;

    ob_start();
    ?>
    <div id="fb-root"></div>
    <script type="text/javascript" id="facebook-jssdk" src="//connect.facebook.net/<?php echo $fb_language; ?>/sdk.js#xfbml=1&appId=<?php echo $fb_app_id; ?>&version=v2.0"></script>
    <div 
        class="fb-comments" 
...
function pbuilder_image($atts, $content = null) {
    ...
    if ($border != 'false') {
        ...
        $css_span .= "border-width:" . $border_width . ";";
        $css_span .= "border-style:" . $border_style . ";";
    }

    if ($shadow != 'false') {
        //$css .= 'box-shadow: '.$shadow_h_shadow.' '.$shadow_v_shadow.' '.$shadow_blur.' '.$shadow_color.';';

        $css_span .= 'box-shadow: ' . $shadow_h_shadow . ' ' . $shadow_v_shadow . ' ' . $shadow_blur . ' ' . $shadow_color . ';';
    }
    global $wpdb;
    $ImageData = $wpdb->get_row("SELECT `post_title`, `post_excerpt` FROM {$wpdb->prefix}posts WHERE guid LIKE '%$content%'");
    ...
    $html = '<span class="frb_image_inner' . ($border != 'false' ? ' frb_image_border ' : '') . ($round != 'false' ? ' frb_image_round ' : '') . '" style="' . $css_span . '" data-bordercolor="' . $border_color . '" data-borderhover="' . $border_hover_color . '">
		<img class="' . ($round != 'false' ? 'frb_image_round' : 'frb_image_flat') . '" src="' . $content . '" title="' . $ImgTitle . '" alt="' . $ImgTitle . '" style="' . $css . '" />
...
function pbuilder_timer($atts) {
    ...
    $timezone = timezone_name_from_abbr('', $offset->timezone, 0);
    $offset = $offset / 3600;
    ...
    if ($timer_style == "Fixed") {
        ...
        ?>
                    ...
                    var now = new Date();
                            ...
                            var diff = (end_date.getTime() - now.getTime()) / 1000;
        <?php
    }
    ?>
    ...
    <?php if ($timer_type == "flipclock") { ?>
        ...
        echo $elements;
        if ($timout_url != '') {
            ?>, callbacks: {stop: function() {window.location.href = '<?php echo $timout_url; ?>'}}<?php } ?>});
        ...
        ?>
                    clock = jQuery('.clock<?php echo $randomId; ?>').mbComingsoon({expiryDate: end_date, speed:100, localization: {days: "<?php echo $days_label; ?>", hours: "<?php echo $hours_label; ?>", minutes: "<?php echo $minutes_label; ?>", seconds: "<?php echo $seconds_label; ?>"} <?php if ($timout_url != '') { ?>, callBack: function(){window.location.href = '<?php echo $timout_url; ?>'}<?php } ?>});
                            /**************************************/
                            // Code added by Asim Ashraf - DevBatch
                            // DateTime: 28 Jan 2015
                            // Code Edit Start
...
function pbuilder_gmap($atts) {
    ...
    $addressurl = "https://maps.google.com/maps?q=$address&output=embed";
    ...
    $iframe = "$extrastyle  <iframe id='iframe" . $randomId . "' style='width:" . $iframe_width . ";height:" . $iframe_height . ";border:0px; " . $overflow . "' src='" . $addressurl . "' " . $scrolling . " seamless frameborder='0'></iframe>";
