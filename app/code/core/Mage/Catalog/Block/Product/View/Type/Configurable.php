        ...
        foreach ($this->getAllowAttributes() as $attribute) {
            ...
            if (is_array($prices)) {
                foreach ($prices as $value) {
                    ...
                    $productxxx = Mage::getModel('catalog/product')->load($productsIndex[0]); 
                    if (count($productsIndex) > 1) {
                    	$pricexxx = $value['pricing_value'];
                    } else {
                    	$pricexxx = $productxxx->getNormalRetail();
                    }
                    
                    if ($productAttribute->getId()=="967") {
                    	$label = $value['label']." m";
                    } else {
                    	$label = $value['label'];
                    }
                    ...
                    $info['options'][] = array(
                        ...
                        //'oldPrice'  => $this->_prepareOldPrice($pricexxx, $value['is_percent']),
