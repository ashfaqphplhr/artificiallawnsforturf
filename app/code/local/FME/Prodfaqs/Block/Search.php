...
class FME_Prodfaqs_Block_Search extends Mage_Core_Block_Template
{
	...
	try {
		...
		try {
				$breadcrumbs->addCrumb('prodfaqs', array('label' => Mage::helper('prodfaqs')->__('Faq search results'), 'title' => Mage::helper('prodfaqs')->__('Faq search results')));
		} catch (Exception $e) {}
    	}
        
        if ($head = $this->getLayout()->getBlock('head')) {
			try {
			    $head->setTitle(Mage::helper('prodfaqs')->__('Faqs Search results'));
			} catch (Exception $e) {}
        }
