...
class FME_Prodfaqs_Block_Productnextpagefaqs extends Mage_Core_Block_Template
{
    public function _prepareLayout()
    {
    	
	try {
    		$current_faqs_product = Mage::registry('current_faqs_product'); //echo "<pre>"; print_r($current_faqs_product);exit;
		...
		$breadcrumbs->addCrumb('product', array('label' => $current_faqs_product->getName(), 'title' => $current_faqs_product->getName(), 'link' => $current_faqs_product->getProductUrl()));
			
		try {
			$breadcrumbs->addCrumb('faqs', array('label' => $this->__($current_faqs_product->getName().' Questions'), 'title' => $this->__($current_faqs_product->getName().' Questions')));
		} catch (Exception $e) {}
    	}
        
        if ($head = $this->getLayout()->getBlock('head')) {
			try {
            $head->setTitle($this->__($current_faqs_product->getName().' Questions'));
			} catch (Exception $e) {}
        }
     ...
     public function getProductNextPageFaqs()     
     {
	
        if (!$this->hasData('nextpagefaqs')) {
	    
            $this->setData('nextpagefaqs', Mage::registry('nextpagefaqs'));
	
        }
	
        return $this->getData('nextpagefaqs');
    }
