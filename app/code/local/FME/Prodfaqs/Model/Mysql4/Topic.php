...
class FME_Prodfaqs_Model_Mysql4_Topic extends Mage_Core_Model_Mysql4_Abstract
{
    public function _construct()
    {    
        // Note that the faqs_id refers to the key field in your database table.
        $this->_init('prodfaqs/topic', 'topic_id');
    }
	
	 protected function _afterLoad(Mage_Core_Model_Abstract $object)
    {
        $select = $this->_getReadAdapter()->select()
            ->from($this->getTable('prodfaqs_store'))
    ...
    protected function _afterSave(Mage_Core_Model_Abstract $object)
    {
        ...
        $this->_getWriteAdapter()->delete($this->getTable('prodfaqs_store'), $condition);
    
        foreach ((array)$object->getData('store_id') as $store) {
            $storeArray = array();
            $storeArray['topic_id'] = $object->getId();
            $storeArray['store_id'] = $store;
            $this->_getWriteAdapter()->insert($this->getTable('prodfaqs_store'), $storeArray);
        }
    ...
    public function checkIdentifier($identifier, $storeId)
    {
        $select = $this->_getReadAdapter()->select()->from(array('main_table'=>$this->getMainTable()), 'topic_id')
            ->join(
                array('store_table' => $this->getTable('prodfaqs_store')),
