...
class FME_Prodfaqs_Adminhtml_TopicController extends Mage_Adminhtml_Controller_action
{

	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('prodfaqs/topic')
	...
	public function editAction() {
		...
		$this->loadLayout();
		$this->_setActiveMenu('prodfaqs/topic');
		$this->_addBreadcrumb(Mage::helper('prodfaqs')->__('Manage Topics'), Mage::helper('prodfaqs')->__('Manage Topics'));
		

		$id = $this->getRequest()->getParam('id');
		if ($id > 0) {
			
			$model  = Mage::getModel('prodfaqs/topic')->load($id);
			Mage::register('topic_data', $model);
		}
		$this->_addContent($this->getLayout()->createBlock('prodfaqs/adminhtml_topic_edit'))
			 ->_addLeft($this->getLayout()->createBlock('prodfaqs/adminhtml_topic_edit_tabs'));
		$this->renderLayout();
	}
	...
	public function saveAction() {
		...
		try {
			...
			// New Cat
			if ($is_new){
				$cat= Mage::getModel('prodfaqs/topic');
				$cat->setData($data);
			...
			// Existing Cat
				...
				$cat=Mage::getModel('prodfaqs/topic')->load($id);
				$cat->setData($data);
			...
			Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('prodfaqs')->__('Topic was successfully saved'));
	...
	public function deleteAction() {
		if( $this->getRequest()->getParam('id') > 0 ) {
			try {
				$model = Mage::getModel('prodfaqs/topic');
        ...
        if(!is_array($faqsIds)) {
            ...
            try {
                foreach ($faqsIds as $faqsId) {
                    $faqs = Mage::getModel('prodfaqs/topic')->load($faqsId);
                    $faqs->delete();
                }
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($faqsIds)) {
            ...
            try {
                ...
                foreach ($faqsIds as $faqsId) {
                    $faqs = Mage::getSingleton('prodfaqs/topic')
                        ->load($faqsId)
    ...
    {
        $fileName   = 'topics.csv';
        $content    = $this->getLayout()->createBlock('prodfaqs/adminhtml_faqs_grid')
            ->getCsv();
