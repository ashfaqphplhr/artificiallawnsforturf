    ...
    implements Mage_Catalog_Model_Product_Configuration_Item_Interface
{
     ...
     * @return float
     */
    public function getCalculationPriceOriginal($optionId=null)
    {
        $price = $this->_getData('calculation_price');
        if (is_null($price)) {
            if ($this->hasOriginalCustomPrice()) {
                ...
                $price = $this->getConvertedPrice($optionId);
            }
            $this->setData('calculation_price', $price);
        }
        return $price;
     ...
     * @return decimal
     */
    public function getPrice($optionId=null)
    {
     ...
     * @return float
     */
    public function getConvertedPrice($optionId=null)
    {
        $price = $this->_getData('converted_price');
        if (is_null($price)) {
            $price = $this->getStore()->convertPrice($this->getPrice($optionId));
