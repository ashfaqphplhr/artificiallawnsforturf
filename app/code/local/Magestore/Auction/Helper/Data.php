...
{	
	...
	public function getProductAuctionIds($store_id = 0)
	{
		$IDs = array();
		$productIds = array();
	...
	public function updateBisStatus($auction_id)
	{
		...
		if ($isSent) Mage::helper('auction/email')->sendAuctionCompleteEmail($auction);
	}
	...
	public function autoUpdateBidStatus($auction_id)
	{
		...
		if($auction->getStatus() == 5) Mage::helper('auction/email')->noticeOutBid($auction);
	}	
	...
	public function getAuctionStatus($auction_id)
	{
		...
		if ($preStatus == 6) $status = 6;
        elseif($preStatus == 5) $status = 5;
