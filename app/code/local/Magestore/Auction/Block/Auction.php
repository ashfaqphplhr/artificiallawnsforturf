...
class Magestore_Auction_Block_Auction extends Mage_Core_Block_Template
{
	...
	{
		return $this->getUrl('auction/index/updateauctioninfo',
								array('id' => $this->getAuction()->getId(), 'tmpl' => 'auctioninfo')); 	
	}
	
	public function getUpdatePriceUrl()
	{
		return $this->getUrl('auction/index/updatepricecondition',
								array('id' => $this->getAuction()->getId()));
	}
	...
	{
		if((int) Mage::getStoreConfig('auction/general/bidder_name_type') == 2)
		{
	...
	public function addToTopLink() {
		if(Mage::helper('auction')->getBidderStatus())
		{
			...
			if($topBlock) 
			{
				$topBlock->addLink($this->__('Auction'),'auction/index',
									'auctionsystem',true,array(),10
								);
			}
		}
	}
