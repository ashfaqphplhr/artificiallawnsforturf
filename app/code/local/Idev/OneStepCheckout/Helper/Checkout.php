...
class Idev_OneStepCheckout_Helper_Checkout extends Mage_Core_Helper_Abstract
{
    ...
    public function saveShippingMethod($shippingMethod, $shippingg=null)
    {
		...
		if(count($shippingg)>0){
			
$this->getOnepage()->getQuote()->getShippingAddress()->setShippingMethod($shippingg['depo_id']);
$this->getOnepage()->getQuote()->getShippingAddress()->setShippingRateByCode($shippingg['price']);
		
        
		$method = $this->getOnepage()->getQuote()->getShippingAddress();
		
	//	Zend_debug::dump($method->getShippingRateByCode());
	//	exit;
