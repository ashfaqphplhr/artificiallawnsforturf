...
class Infortis_Ultimo_Block_Navigation extends Mage_Core_Block_Template
{
    ...
    {
          $nail = Mage::getModel('catalog/category')->load($category->getId())->getThumbnail();
		  return Mage::getBaseUrl('media').'catalog/category/' . $nail;		  
    }
	public function getThumbnailUrlCheck($category)
    {
          $nail = Mage::getModel('catalog/category')->load($category->getId())->getThumbnail();
		  return $nail;		  
    }
        ...
        $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
    {
		...
		if($level == 1 && $this->getThumbnailUrlCheck($category) != ""){
			$html[] = '<img src="'.$this->getThumbnailUrl($category).'" height="80" width="80" alt="' . $this->escapeHtml($category->getName()) . '"/>';
		}
        $html[] = '</a>';
